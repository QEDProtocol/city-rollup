...
use city_common::cli::args::RollupProvingWorkerArgs;

pub async fn run(args: &RollupProvingWorkerArgs) -> anyhow::Result<()> {
    loop {
...
pub async fn run(args: &SequencerArgs) -> anyhow::Result<()> {
    let (pk, vk, mut rng, signing_key, bitcoincore_rpc, rpc) = l2transfer::run(&InitializerArgs {
