...
use crate::store::kvq::traits::{KVQBinaryStore, KVQPair, KVQSerializable, KVQStoreAdapter};

pub struct KVQStagedAdapter<S: KVQBinaryStore, K: KVQSerializable, V: KVQSerializable> {
    _s: PhantomData<S>,
    _k: PhantomData<K>,
    _v: PhantomData<V>,
}
    ...
    for KVQStagedAdapter<S, K, V>
{
    ...
    fn get_many_exact(s: &S, keys: &[K]) -> anyhow::Result<Vec<V>> {
        let keys_bytes = keys.iter().map(|k| k.to_bytes()).collect_vec();
        let values_bytes = s.get_many_exact(&keys_bytes)?;
        let values = values_bytes.iter().map(|r| V::from_bytes(r)).collect_vec();
        Ok(values)
    }
