...
pub struct Sha256AirParametersStandard<F: RichField, P: CubicParameters<F>>(
    std::marker::PhantomData<(F, P)>,
);

impl<F: RichField, P: CubicParameters<F>> AirParameters for Sha256AirParametersStandard<F, P> {
    type Field = F;
    type CubicParams = P;
...
impl SimpleSha256AcceleratorPlannerGadget {
    ...
    ) -> SmartSha256AcceleratorGadget<SHA256, Sha256AirParametersStandard<C::F, P>, C, D, 64>
    where
        <C as GenericConfig<D>>::Hasher: AlgebraicHasher<<C as GenericConfig<D>>::F>,
    {
        SmartSha256AcceleratorGadget::<SHA256, Sha256AirParametersStandard<C::F, P>, C, D, 64>::finalize_planner(
            builder,
            self
        )
    }
    ...
    C::Hasher: AlgebraicHasher<C::F>,
{
    pub fn finalize_planner(
        builder: &mut CircuitBuilder<C::F, D>,
        planner: SimpleSha256AcceleratorPlannerGadget,
    ) -> Self {
        ...
        Self::add_virtual_to_internal(builder, acc_config, Some(planner))
    }
    fn add_virtual_to_internal(
        builder: &mut CircuitBuilder<C::F, D>,
        acc_config: HashAcceleratorConfig,
        planner: Option<SimpleSha256AcceleratorPlannerGadget>,
    ) -> Self {
        ...
        let (stark_proof_target, stark_public_inputs) = if planner.is_some() {
            ...
            let padded_preimage_targets = planner
                .planned_hashes
                .iter()
                .flat_map(|x| {
                ...
                .collect::<Vec<Target>>();
            let flattened_digest_targets = planner
                .planned_hashes
                .iter()
