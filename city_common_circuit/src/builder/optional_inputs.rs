...
pub trait CircuitBuilderOptionalInputs<F: RichField + Extendable<D>, const D: usize> {
    ...
    fn add_virtual_hashes_if_none(
        &mut self,
        hash_targets: Option<Vec<HashOutTarget>>,
        length: usize,
    ) -> Vec<HashOutTarget>;

    fn add_virtual_target_if_none_op(
        &mut self,
        target: Option<Target>,
        option_flags: &mut u64,
        flag: u64,
    ) -> Target;
    fn add_virtual_hash_if_none_op(
        &mut self,
        hash_target: Option<HashOutTarget>,
        option_flags: &mut u64,
        flag: u64,
    ) -> HashOutTarget;
    fn add_virtual_bool_if_none_op(
        &mut self,
        bool_target: Option<BoolTarget>,
        option_flags: &mut u64,
        flag: u64,
    ) -> BoolTarget;
