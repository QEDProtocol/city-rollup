...
// Get a user by id
fn get_user_by_id(args: HashMap<String, Value>, context: &mut ReplContext) -> Result<Option<String>> {
  ...
  let result = context.city_rpc.get_user_by_id_sync(checkpoint_id, user_id)?;
...
// Get a deposit by id
fn get_deposit_by_id(args: HashMap<String, Value>, context: &mut ReplContext) -> Result<Option<String>> {
  ...
  let result = context.city_rpc.get_deposit_by_id_sync(checkpoint_id, deposit_id)?;
...
// Get a block by id
fn get_block_by_id(args: HashMap<String, Value>, context: &mut ReplContext) -> Result<Option<String>> {
  let checkpoint_option = args.get("checkpoint");
  let checkpoint_id: u64 = if checkpoint_option.is_some() {
    checkpoint_option.unwrap().convert()?
  }else{
...
fn get_block_deposit_address_by_id(args: HashMap<String, Value>, context: &mut ReplContext) -> Result<Option<String>> {
  let checkpoint_option = args.get("checkpoint");
  let checkpoint_id: u64 = if checkpoint_option.is_some() {
    checkpoint_option.unwrap().convert()?
  }else{
  ...
  let result = context.city_rpc.get_city_block_deposit_address_string_sync(checkpoint_id)?;
...
fn get_utxos(args: HashMap<String, Value>, context: &mut ReplContext) -> Result<Option<String>> {
  ...
  let result = context.btc_link_rpc.get_utxos(BTCAddress160::try_from_string(&address)?)?;
...
fn faucet(args: HashMap<String, Value>, context: &mut ReplContext) -> Result<Option<String>> {
  ...
  let result = context.btc_link_rpc.fund_address_from_random_p2pkh_address(BTCAddress160::try_from_string(&address)?, amount)?;



  Ok(Some(format!("{{\"txid\": \"{}\"}}", result.to_hex_string())))
}
...
fn spend_all(args: HashMap<String, Value>, context: &mut ReplContext) -> Result<Option<String>> {
  ...
  let txid = send_entire_balance_simple_p2pkh(&context.btc_link_rpc, &wallet, from.address, to, fee)?;
...
fn random_dogecoin_wallet(_args: HashMap<String, Value>, _context: &mut ReplContext) -> Result<Option<String>> {
...
pub async fn run(args: RPCReplArgs) -> Result<()> {
  ...
  let mut repl = Repl::new(ReplContext::new(&args.rpc_address, &args.bitcoin_rpc, &args.electrs_api))
    ...
    Command::new("exit", exit_repl)
        .with_help("exits the repl")
  );
