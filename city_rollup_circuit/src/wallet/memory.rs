...
use city_common_circuit::{
  ...
  wallet::zk::{MemoryZKSignatureWallet, SimpleZKSignatureWallet, ZKSignatureBasicWalletProvider, ZKSignatureWalletProvider},
};
use city_crypto::{
  hash::{
...
#[derive(Clone)]
pub struct CityMemoryWallet<C: GenericConfig<D> + 'static, const D: usize>
where
...
impl<C: GenericConfig<D> + 'static, const D: usize> CityMemoryWallet<C, D>
where
  C::Hasher: AlgebraicHasher<C::F>,
{
  pub fn new() -> Self {
  ...
  pub fn zk_sign_with_private_key(
      ...
      self.zk_wallet.zk_sign(private_key, action_hash)
  }
  ...
  pub fn zk_secp256k1_from_signature(
      ...
      self.secp256k1_circuit.as_ref().unwrap().prove(signature)
  }
