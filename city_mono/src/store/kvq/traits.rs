...
pub trait KVQSerializable: Clone + Eq + PartialEq + Debug + Hash {
...
impl<T: KVQBinaryStore> KVQStoreAdapter<T, Vec<u8>, Vec<u8>> for T{
    fn get_exact(s: &T, key: &Vec<u8>) -> anyhow::Result<Vec<u8>> {
        Self::b_get_exact(&s, key)
    }

    fn get_many_exact(s: &T, keys: &[Vec<u8>]) -> anyhow::Result<Vec<Vec<u8>>> {
        Self::b_get_many_exact(&s, keys)
    }

    fn set(s: &mut T, key: Vec<u8>, value: Vec<u8>) -> anyhow::Result<()> {
        Self::b_set(s, key, value)
    }

    fn set_many_ref<'a>(s: &mut T, items: &[KVQPair<&'a Vec<u8>,&'a Vec<u8>>]) -> anyhow::Result<()> {
        Self::b_set_many_ref(s, items)
    }

    fn set_many_vec(s: &mut T, items: Vec<KVQPair<Vec<u8>, Vec<u8>>>) -> anyhow::Result<()> {
        Self::b_set_many_vec(s, items)
    }

    fn delete(s: &mut T, key: &Vec<u8>) -> anyhow::Result<bool> {
        Self::delete(s, key)
    }

    fn delete_many(s: &mut T, keys: &[Vec<u8>]) -> anyhow::Result<Vec<bool>> {
        Self::delete_many(s, keys)
    }

    fn get_leq(s: &T, key: &Vec<u8>, fuzzy_bytes: usize)->anyhow::Result<Option<Vec<u8>>> {
        Self::get_leq(s, key, fuzzy_bytes)
    }

    fn get_leq_kv(s: &T, key: &Vec<u8>, fuzzy_bytes: usize) -> anyhow::Result<Option<KVQPair<Vec<u8>,Vec<u8>>>> {
        Self::get_leq_kv(s, key, fuzzy_bytes)
    }

    fn get_many_leq(s: &T, keys: &Vec<u8>, fuzzy_bytes: usize)->anyhow::Result<Option<Vec<u8>>> {
        Self::get_many_leq(s, keys, fuzzy_bytes)
    }

    fn get_many_leq_kv(s: &T, keys: &Vec<u8>, fuzzy_bytes: usize) -> anyhow::Result<Vec<Option<KVQPair<Vec<u8>,Vec<u8>>>>> {
        Self::get_many_leq_kv(s, keys, fuzzy_bytes)
    }
}
*/
